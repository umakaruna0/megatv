...
class OLERead {
        ...
        function read($sFileName){
                ...
                if (mb_substr($this->data, 0, 8, 'windows-1251') != IDENTIFIER_OLE) {
                        $this->error = 1;
                        return false;
                }
        ...
        function __readData($bl) {
                ...
                while ($block != -2)  {
                        ...
                        $data = $data.mb_substr($this->data, $pos, BIG_BLOCK_SIZE, 'windows-1251');
        ...
        function __readPropertySets(){
                ...
                while ($offset < strlen($this->entry)) {
                        $d = mb_substr($this->entry, $offset, PROPERTY_STORAGE_BLOCK_SIZE, 'windows-1251');
        ...
        function getWorkBook(){
                if ($this->props[$this->wrkbook]['size'] < SMALL_BLOCK_THRESHOLD){
                        ...
                        while ($block != -2) {
                                  ...
                                  $streamData .= mb_substr($rootdata, $pos, SMALL_BLOCK_SIZE, 'windows-1251');
                        ...
                        while ($block != -2) {
                          $pos = ($block + 1) * BIG_BLOCK_SIZE;
                          $streamData .= mb_substr($this->data, $pos, BIG_BLOCK_SIZE, 'windows-1251');
                          $block = $this->bigBlockChain[$block];
                        }
                        return $streamData;
                }
...
class Spreadsheet_Excel_Reader {
    ...
    var $colors = Array(
        ...
        function read16bitstring($data, $start) {
                ...
                return mb_substr($data, $start, $len, 'windows-1251');
        }
        ...
        function _parse() {
                ...
                if ($mb_substreamType != SPREADSHEET_EXCEL_READER_WORKBOOKGLOBALS){
                        return false;
                }
                        ...
                        switch ($code) {
                                case SPREADSHEET_EXCEL_READER_TYPE_SST:
                                        ...
                                        for ($i = 0; $i < $uniqueStrings; $i++) {
                                                ...
                                                if ($spos + $len < $limitpos) {
                                                        $retstr = mb_substr($data, $spos, $len, 'windows-1251');
                                                        $spos += $len;
                                                }
                                                else{
                                                        // found countinue
                                                        $retstr = mb_substr($data, $spos, $limitpos - $spos, 'windows-1251');
                                                        ...
                                                        while ($charsLeft > 0){
                                                                ...
                                                                if ($asciiEncoding && ($option == 0)) {
                                                                        ...
                                                                        $retstr .= mb_substr($data, $spos, $len);
                                                                        $charsLeft -= $len;
                                                                        $asciiEncoding = true;
                                                                }
                                ...
                                case SPREADSHEET_EXCEL_READER_TYPE_FORMAT:
                                        ...
                                        if ($version == SPREADSHEET_EXCEL_READER_BIFF8) {
                                                ...
                                                if (ord($data[$pos+8]) == 0){
                                                        $formatString = mb_substr($data, $pos+9, $numchars, 'windows-1251');
                                                } else {
                                                        $formatString = mb_substr($data, $pos+9, $numchars*2, 'windows-1251');
                                                }
                                        } else {
                                                $numchars = ord($data[$pos+6]);
                                                $formatString = mb_substr($data, $pos+7, $numchars*2, 'windows-1251');
                                        }
                                ...
                                case SPREADSHEET_EXCEL_READER_TYPE_FONT:
                                                ...
                                                if ((ord($data[$pos+19]) & 1) == 0){
                                                    $font = mb_substr($data, $pos+20, $numchars, 'windows-1251');
                                                } else {
                                                    $font = mb_substr($data, $pos+20, $numchars*2, 'windows-1251');
...
//                                              $bgcolor = ($bgi & 0x3f80) >> 7;
                                ...
                                case SPREADSHEET_EXCEL_READER_TYPE_BOUNDSHEET:
                                                ...
                                                if ($version == SPREADSHEET_EXCEL_READER_BIFF8){
                                                        ...
                                                        if ($chartype == 0){
                                                                $rec_name       = mb_substr($data, $pos+12, $rec_length, 'windows-1251');
                                                        } else {
                                                                $rec_name       = $this->_encodeUTF16(mb_substr($data, $pos+12, $rec_length*2, 'windows-1251'));
                                                        }
                                                }elseif ($version == SPREADSHEET_EXCEL_READER_BIFF7){
                                                                $rec_name       = mb_substr($data, $pos+11, $rec_length, 'windows-1251');
                                                }
        ...
        function _parsesheet($spos) {
                ...
                $mb_substreamType = ord($data[$spos + 6]) | ord($data[$spos + 7])<<8;

                if (($version != SPREADSHEET_EXCEL_READER_BIFF8) && ($version != SPREADSHEET_EXCEL_READER_BIFF7)) {
                        return -1;
                }

                if ($mb_substreamType != SPREADSHEET_EXCEL_READER_WORKSHEET){
                        return -2;
                }
                $spos += $length + 4;
                while($cont) {
                        ...
                        switch ($code) {
                                ...
                                case SPREADSHEET_EXCEL_READER_TYPE_NUMBER:
                                        $row    = ord($data[$spos]) | ord($data[$spos+1])<<8;
                                        $column = ord($data[$spos+2]) | ord($data[$spos+3])<<8;
                                        $tmp = unpack("ddouble", mb_substr($data, $spos + 6, 8/*, 'windows-1251'*/)); // It machine machine dependent                                       
                                        if ($this->isDate($spos)) {
                                                $numValue = $tmp['double'];
                                        }
                                        else {
                                        ...
                                        //$tmp2 = unpack("ddouble", mb_substr($data, $spos + 6, 8));
                                        //echo "<pre>"; print_r($tmp2); echo "</pre>";
                                        //echo $tmp."<br />";
                                        //echo $numValue;
                                        $info = $this->_getCellDetails($spos,$numValue,$column);
                                        $info["string"] = $numValue;
                ...
                case SPREADSHEET_EXCEL_READER_TYPE_STRING:
                        ...
                        // See if string contains formatting information
                                                ...
                                                $len = ($asciiEncoding)?$numChars : $numChars*2;
                                                $retstr =mb_substr($data, $xpos, $len, 'windows-1251');
                                                $xpos += $len;
                                                $retstr = ($asciiEncoding)? $retstr : $this->_encodeUTF16($retstr);
                                        }
                                        elseif ($version == SPREADSHEET_EXCEL_READER_BIFF7){
                                                ...
                                                $numChars =ord($data[$xpos]) | (ord($data[$xpos+1]) << 8);
                                                $xpos += 2;
                                                $retstr =mb_substr($data, $xpos, $numChars, 'windows-1251');
                                        }
                                ...
                                case SPREADSHEET_EXCEL_READER_TYPE_LABEL:
                                        $row    = ord($data[$spos]) | ord($data[$spos+1])<<8;
                                        $column = ord($data[$spos+2]) | ord($data[$spos+3])<<8;
                                        $this->addcell($row, $column, mb_substr($data, $spos + 8, ord($data[$spos + 6]) | ord($data[$spos + 7])<<8, 'windows-1251'));
                                        break;
                                ...
                                case SPREADSHEET_EXCEL_READER_TYPE_HYPER:
                                        ...
                                        if (($flags & 1) > 0 ) {   // is a type we understand
                                                ...
                                                if (($flags & 0x14) == 0x14 ) {   // has a description
                                                        ...
                                                        $udesc = mb_substr($this->data, $spos + $uloc, $descLen * 2, 'windows-1251');
                                                        $uloc += 2 * $descLen;
                                                }
